...
class AIChatDashboard extends Page implements HasForms
{
    ...
    protected static string $view = 'filament.pages.ai-chat-dashboard';
    ...
    {
        return [
            Forms\Components\Section::make('Chat Configuration')
                ->schema([
                    Forms\Components\Select::make('selectedProvider')
                        ->label('AI Provider')
                        ->options([
                            'openai' => 'OpenAI',
            ...
            Forms\Components\Section::make('Your Message')
                ->schema([
    ...
    public function sendMessage(): void
    {
        ...
        try {
            ...
            Notification::make()
                ...
                ->body("Response received from {$this->selectedProvider}")
                ->success()
                ->send();
    ...
    public function startStream(): void
    {
        ...
        try {
            $provider = app(AIProviderContract::class);
            $stream = $provider->stream($this->message, [
                'model' => $this->selectedModel,
                'max_tokens' => $this->maxTokens,
                'temperature' => $this->temperature,
            ]);
